...
用词必须使用两字及以上的完整形式。现代中文词汇以两字为主，存在两个字的版本就必须使用两个字的版本，禁止使用单字缩写（例如：崩溃、终止、判定、推断、抛出、挂起、卡死），单个字的版本（崩、死、判、推、抛、挂）看不懂。代码标识符保持英文原名。禁止生造名词。例如“两个字的版本”也不允许被缩减为“两字版本”，“单个字的版本”也不允许被缩减为“单字版本”。描述具体操作时使用完整的动宾结构，说明动作与对象，禁止使用自造的缩略说法，例如：把“用新版本动态库替换 `_vllm_fa3_C.abi3.so` 共享库文件”说成“换库”。
不允许使用“落地”“钉死”“对齐”等非技术名词、显然有其他可以代替的词语的黑话。我并没有在互联网大厂工作过，看不懂这种黑话。使用正常的、月薪4000的普通人可以看懂的，在简单中文里常用的词汇。
不允许使用“栈”字（“技术栈”“模型栈”等），直接说明具体事物，例如“使用的技术”“全部模型”。
代码标识符保持英文原名，**严厉禁止翻译**。**严厉禁止翻译**。
...
上述规则适用于所有的文字内容，包括文档、注释、对话等等。

## 行为
除非显式要求，否则：
- **禁止**使用try-except进行import。如果一个库是需要的，你必须直接import。
- **禁止**擅自进入plan mode
- **禁止**用Git回滚任何代码（严厉禁止。如果做了，你将会遭受毁灭性打击）
...
JUCE 代码中，含有非 ASCII 字符的字符串字面量必须添加 u8 前缀。juce::String (const char\*) 构造函数只接受纯 ASCII 输入，多字节字符必须经由 char8_t 字面量、juce::String::fromUTF8 或 juce::CharPointer_UTF8 进入 juce::String。tools/check-utf8-literals.py 用于检查该规则，修改 plugin/Source 后必须运行。
...
构建流程：
- `python build.py`。**不允许**带任何变量、输出重定向、输入、输出pipe等乱七八糟的前缀后缀。
- 运行`tools/check-utf8-literals.py`，确保中文不乱码。
- 在开发插件的时候，遇到前后不兼容的情况是正确的，宿主也应该正确的提示挂了，而不是由我们去做一大堆乱七八糟的兼容，我们不做任何的兼容，直接当场失败，让宿主提示挂了，我们再重新去添加插件即可。

在修复任何bug的时候，你都没有资格把你的猜测当作根因，除非这个猜测被我确认，或者被真正的实证数据确认。
...
后续日志的基础设施就不要删了，后面日志影响到运行速度了，我会再要求你删日志，不需要主动删除。

开发完功能或者修复完bug，如果没有要求你测试，那么只进行安装，不进行测试。也不要编写测试。

出现了bug：
- 先去查看最新改动的Studio One的.song工程，查阅日志，定位问题
- **禁止**猜测DAW的任何行为。一切必须依据日志。